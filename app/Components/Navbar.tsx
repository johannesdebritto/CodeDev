              ...
              <li key={idx} className="relative group">
                ...
                {item === "Layanan" ? (
                  <button
                    ...
                    className="hover:text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-blue-500 text-gray-700 transition-all duration-300 group-hover:scale-105 focus:outline-none"
                  >
                    Layanan
                  </button>
                ) : (
            ...
            {item}
                ...
                {item === "Layanan" && isDropdownOpen && (
                  <ul className="absolute left-[50%] transform -translate-x-1/2 top-full mt-2 w-48 bg-gradient-to-r from-purple-600 to-blue-500 shadow-lg rounded-lg z-10">
                    {[{ name: "Web Developer", path: "/LayananWebPage" }, { name: "Android Developer", path: "/LayananAndroidPage" }, { name: "UI/UX Design", path: "/LayananUXUIPage" }].map((service, idx) => (
                      <li key={idx}>
                        <Link
                          href={service.path}
                          onClick={closeDropdown}
                          className="block px-6 py-2 text-white text-sm hover:bg-gray-700 transition-all duration-300"
                        >
                          {service.name}
                        </Link>
                      </li>
    ...
    <ul className="flex flex-col items-center space-y-4 text-center text-gray-800 font-medium w-full px-4">
      ...
      <li className="w-full">
        <Link
          href="/"
          onClick={closeMenu}
          className="block text-white transition-all duration-300 text-2xl hover:scale-105"
        >
          Home
        </Link>
      </li>
      ...
      <li className="w-full">
        <button
          onClick={toggleDropdown}
          className="block w-full text-white transition-all duration-300 text-2xl hover:scale-105 text-center"
        >
          Layanan
        </button>
        ...
        {isDropdownOpen && (
          <ul className="mt-2 w-full bg-gradient-to-r from-purple-600 to-blue-500 shadow-lg rounded-lg space-y-2 px-4 py-2">
            {[
              ...
              { name: "UI/UX", route: "/LayananUXUIPage" },
            ].map((service, idx) => (
              <li key={idx}>
                <Link
                  href={service.route}
